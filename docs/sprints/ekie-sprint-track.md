...
Handbook mapping: Chapter 12 (Workflow Orchestration), Chapter 5 (workflow relationship).

### Sprint Objective
Orchestrate the end-to-end ingestion workflow with checkpoints, retries, and lineage-aware replay.

### Scope
1. Workflow definition across sync, transform, intelligence, chunk, embed, publish.
2. Checkpointing, dead-letter handling, and lease recovery.
3. Event-driven execution and idempotent task processing.
4. Reconciliation and lineage-aware replay.

### Out Of Scope
1. Multi-region failover execution (deployment readiness sprint).

### Stories
1. EKIE-S7-1 Workflow definition and task orchestration.
2. EKIE-S7-2 Checkpointing and resume-from-checkpoint recovery.
3. EKIE-S7-3 Dead-letter handling and retry policy.
4. EKIE-S7-4 Lineage-aware replay and reconciliation.

### Deliverables
1. Workflow orchestration model and task graph.
...
| Story ID | Story Name | Priority | Owner | Estimate | Depends On | Done Evidence |
| --- | --- | --- | --- | --- | --- | --- |
| EKIE-S7-1 | Workflow definition and task graph | High | Workflow Owner | T-Shirt M | EKIE-S6 Exit | Approved workflow model |
| EKIE-S7-2 | Checkpoint and resume recovery | High | Reliability Owner | T-Shirt M | EKIE-S7-1 | Recovery validation |
