...
Binds ``tenant_id`` and ``correlation_id`` to the current execution scope so
every log record and trace can carry them without threading arguments through
every function. Values are stored in :class:`contextvars.ContextVar` objects,
which are safe across ``async`` tasks and threads.
"""
...
_tenant_id: ContextVar[str | None] = ContextVar("ekie_tenant_id", default=None)
_correlation_id: ContextVar[str | None] = ContextVar("ekie_correlation_id", default=None)
