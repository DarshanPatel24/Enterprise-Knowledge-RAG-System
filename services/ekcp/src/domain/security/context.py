"""Security context ingress validation for every governed conversation request.

Establishes the pre-execution security gate: a request must carry a valid
:class:`SecurityContext` (identity and clearance) before intent gating, context
assembly, or generation. The same validated context is later propagated to EKRE
on downstream retrieval requests.
"""
        ...
        if not raw:
            if self._require_security_context:
                raise SecurityError(
                    ...
                    "a security context is required for every governed request",
                )
            return None
