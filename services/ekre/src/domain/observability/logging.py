"""Structured JSON logging baseline for EKRE.

Emits one JSON object per log record and automatically enriches every record
with the ``tenant_id``, ``correlation_id``, and ``query_id`` bound to the
current context. This is the observability substrate every retrieval stage
builds on.
"""
...
def configure_logging(service_name: str = "ekre", log_level: str = "INFO") -> None:
